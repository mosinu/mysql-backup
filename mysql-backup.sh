...
PATH=$PATH:/usr/local/bin:/usr/bin:/bin:/sbin:/usr/sbin:/usr/local/sbin:/usr/local/mysql/bin
...
then
	...
	if [ -s "$LOGERR" ]
		then
			...
			echo
			echo "###### WARNING ######"
			echo "Errors reported during Backup..Backup failed"
